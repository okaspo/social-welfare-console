-- ==========================================
-- GovAI Console: Quick Start Test Data
-- ==========================================
-- このスクリプトで最小限のテストデータを投入します
...
INSERT INTO plan_limits (id, name, o1_monthly_quota, precision_check_quota, ai_chat_enabled, word_export_enabled, email_sending_enabled, subsidy_matching_enabled)
VALUES 
...
-- ==========================================
-- 2. 自分の組織をProプランに変更（テスト用）
-- ==========================================
-- ⚠️ 以下のクエリを実行する前に、自分のorganization_idを確認してください

-- 自分のorganization_idを確認
SELECT id, name, plan_id FROM organizations;

-- Proプランに変更（your-org-id を実際のIDに置換）
...
-- プラン確認
SELECT * FROM plan_limits ORDER BY name;

-- 助成金確認
SELECT title, provider, category, amount_max, application_period_end 
FROM subsidies 
WHERE is_active = true
ORDER BY application_period_end;

-- アシスタント確認
SELECT name, entity_type, color_primary 
FROM assistant_profiles
ORDER BY entity_type;

-- 組織確認
SELECT id, name, plan_id, entity_type 
FROM organizations
...
-- ==========================================
-- 次のステップ:
-- 1. 自分のorganizationをProプランに変更（上記のUPDATE文）
-- 2. http://localhost:3000 でログイン
-- 3. AIチャット、助成金マッチング等をテスト
-- ==========================================
